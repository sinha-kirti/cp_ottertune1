...
###-------Addition Section : 25th Dec 2018 ---
from analysis.iris import Iris
###------Section end--------------
...
def gen_random_data(knobs):
    ...
    for name, metadata in list(knobs.items()):
        if metadata.vartype == VarType.BOOL:
            ...
            raise Exception('Unknown variable type: {}'.format(metadata.vartype))
...
def map_workload(target_data):
    ...
    #-------------ZKS_code:begin--------------------
    print(target_data)
    print("no. of keys in target_data : ")
    print(len(target_data))
    print("Printing all the key - value pairs in target_data")
    for x, y in target_data.items():
        print(x, y) 
    #-------------ZKS_code:end----------------------
    if target_data['bad']:
    ...
    for unique_workload in unique_workloads:
        ...
        #---------ZKS_code------------------------
        print("Global Ranked Knobs = ", global_ranked_knobs)
        print("Global Pruned Metrics = ", global_pruned_metrics)
        print("Ranked Knob Idxs = ", ranked_knob_idxs)
        print("Pruned Metric Idxs = ", pruned_metric_idxs)
        print("unique_workload = ", unique_workload)
        print("workload_data[unique_wokload]=", workload_data[unique_workload])
        #-----------ZKS_code----------------------
    ...
    for workload_id, workload_entry in list(workload_data.items()):
        ...
        #---------ZKS_code------------------------
        print("X_workload = ", X_workload)
        print("X_scaled = ", X_scaled)
        print("y_workload = ", y_workload)
